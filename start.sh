...
PORT=5081 SUPABASE_URL="$SURL" SUPABASE_SERVICE_ROLE_KEY="$SKEY" nohup bash -c 'ulimit -n 65536 2>/dev/null; exec npm run dev' > "$DIR/youbot.log" 2>&1 &
echo $! > "$DIR/youbot.pid"

# 2) Start Next.js UI on port 5080 (user-facing)
echo "🎨 Starting Next.js UI on :5080 (Turbopack)..."
cd "$DIR/web-ui"
PORT=5080 WATCHPACK_POLLING=true NEXT_PUBLIC_SUPABASE_URL="$SURL" NEXT_PUBLIC_SUPABASE_ANON_KEY="$SANON" nohup bash -c 'ulimit -n 65536 2>/dev/null; exec npm run dev' > "$DIR/web-ui.log" 2>&1 &
echo $! > "$DIR/web-ui.pid"

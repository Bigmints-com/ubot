#!/bin/bash
#
# Deploy UBOT Webchat Relay to Google Cloud Run
#
# Usage:
...
SERVICE_NAME="ubot-webchat-relay"
...
# Generate a random secret if not provided. Reuse existing if found in UBOT config.
if [[ -z "$BOT_SECRET" ]]; then
  UBOT_CONFIG="$HOME/.ubot/config.json"
  if [[ -f "$UBOT_CONFIG" ]]; then
    EXISTING=$(python3 -c "import json; cfg=json.load(open('$UBOT_CONFIG')); print(cfg.get('channels',{}).get('webchat',{}).get('bot_secret',''))" 2>/dev/null || echo "")
    if [[ -n "$EXISTING" ]]; then
      BOT_SECRET="$EXISTING"
...
echo ""
echo "🌐 Deploying UBOT Webchat Relay"
...
echo "   ── Next Steps ──────────────────────────────────"
echo ""
echo "   1. Add to your UBOT config (~/.ubot/config.json):"
echo ""
echo "      \"channels\": {"
echo "        \"webchat\": {"
echo "          \"enabled\": true,"
...
# ── Auto-update UBOT config ──────────────────────────────────────────────
UBOT_CONFIG="$HOME/.ubot/config.json"
if [[ -f "$UBOT_CONFIG" ]]; then
  echo "📝 Updating UBOT config..."
  # Use python3 to merge webchat config
  python3 -c "
import json, sys
with open('$UBOT_CONFIG', 'r') as f:
    cfg = json.load(f)
...
with open('$UBOT_CONFIG', 'w') as f:
    json.dump(cfg, f, indent=2)
print('   Updated ~/.ubot/config.json with webchat relay settings')
  "
  echo ""
  echo "   Restart UBOT to connect: ubot restart"
fi

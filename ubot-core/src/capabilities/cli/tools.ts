...
import { logCapability } from "./capability-log.js";
...
const CLI_TOOLS: ToolDefinition[] = [
  {
    name: "cli_run",
    description:
      "Start a CLI coding session to build or modify a project. Spawns an AI coding assistant (e.g. Gemini CLI) with the given prompt in a sandboxed workspace directory. Returns the session ID and initial status. Use cli_status to check progress.",
    parameters: [
      {
        name: "prompt",
        type: "string",
        description:
          'The task prompt for the coding CLI (e.g. "Build a React landing page with a hero section and contact form")',
        required: true,
      },
      {
        name: "project_name",
        type: "string",
        description:
          "Optional project name — used as the working directory name. Defaults to auto-generated name.",
        required: false,
      },
    ],
  },
  {
    name: "cli_status",
    description:
      "Check the status and output of a CLI coding session. Returns the latest output lines and whether the session is still running.",
    parameters: [
      {
        name: "session_id",
        type: "string",
        description: "The session ID returned by cli_run",
        required: true,
      },
      {
        name: "from_line",
        type: "number",
        description:
          "Start reading output from this line number (0-based). Use to get incremental updates. Default: 0",
        required: false,
      },
    ],
  },
  ...
  },
  {
    ...
    description:
      "List all CLI coding sessions with their status, provider, and project name.",
    parameters: [],
  },
  {
    name: "cli_send_input",
    description:
      "Send text input to a running CLI session (e.g. to answer prompts or provide feedback).",
    parameters: [
      {
        name: "session_id",
        type: "string",
      ...
      },
      {
        name: "input",
        type: "string",
        description: "The text to send to the CLI session stdin",
        required: true,
      },
    ],
  },
  {
    ...
    description:
      "Test a staged custom tool module before promoting it to live. Validates file existence, import, ToolModule interface conformance, and tool naming conventions. Use after cli_run generates a module in custom/staging/.",
    parameters: [
      {
        name: "module_name",
        type: "string",
        description: "Name of the module directory in custom/staging/",
        required: true,
      },
    ],
  },
  {
    ...
    description:
      "Promote a tested custom module from staging to live. Copies from custom/staging/<name>/ to custom/modules/<name>/ and hot-reloads it into the running system. The module's tools become immediately available without restart.",
    parameters: [
      {
        name: "module_name",
        type: "string",
        description: "Name of the module to promote",
        required: true,
      },
    ],
  },
  {
    ...
    description:
      "List all custom tool modules — both staged (in-progress) and live (active). Shows status and tool counts.",
    parameters: [],
  },
  {
    name: "cli_triage",
    description:
      "ONLY use this when the user explicitly asks to BUILD or ADD a new capability/tool/integration that does not yet exist. Do NOT call this for tasks you can already perform with existing tools (web browsing, email, calendar, messaging, etc). Call this BEFORE cli_run to evaluate feasibility.",
    parameters: [
      {
        name: "request",
        type: "string",
        description:
          'The NEW capability to build (e.g. "add weather checking tool", "build a crypto price tracker")',
        required: true,
      },
    ],
  },
  {
    ...
    description:
      "Delete a custom module from staging, live, or both. Use for cleanup after failed builds or to remove unwanted capabilities.",
    parameters: [
      {
        name: "module_name",
        type: "string",
      ...
      },
      {
        ...
        type: "string",
        description:
          'Where to delete from: "staging", "live", or "both" (default: "both")',
        required: false,
      },
    ],
  },
...
const cliToolModule: ToolModule = {
  ...
  register(registry: ToolRegistry, _ctx: ToolContext) {
    const lazyCliService = async () => {
      ...
      const cli = config.capabilities?.cli;
      const provider = cli?.default || "gemini";
      const providerCfg = cli?.providers?.[provider];

      if (!providerCfg || providerCfg.enabled === false) {
        throw new Error(
          "CLI capability is disabled. Enable it from the CLI page in the dashboard.",
        );
      }

      const { getCliService } = await import("./service.js");
      return getCliService({
        provider,
        workDir: cli?.workDir || "custom/staging",
    ...
    registry.register("cli_run", async (args) => {
      try {
        ...
        if (!rawPrompt)
          return toolResult("cli_run", false, 'Missing "prompt" parameter');
...
`;
          ...
          try {
            ...
            const duration =
              completedSession.endedAt && completedSession.startedAt
                ...
                : 0;
            const msg =
              `${emoji} **CLI session "${completedSession.projectName}" ${completedSession.status}** (${duration}s)\n\n` +
            ...
            store.addMessage("web-console", "assistant", msg, {
              source: "cli-notification",
            });
            log.info(
              "CLI",
              `Sent completion notification for "${completedSession.projectName}"`,
            );
          } catch (err: any) {
            log.error(
              "CLI",
              `Failed to send completion notification: ${err.message}`,
            );
          }
        });
    ...
    registry.register("cli_status", async (args) => {
      try {
        ...
        if (session.status === "stopped" || session.exitCode !== undefined) {
          ...
          const indexPath = await import("path").then((p) =>
            p.join(session.workDir, "index.ts"),
          );
    ...
    registry.register("cli_send_input", async (args) => {
      try {
        ...
        return toolResult(
          "cli_send_input",
          true,
          `Input sent to session "${sessionId}".`,
        );
      } catch (err: any) {
    ...
    registry.register("cli_test_module", async (args) => {
      try {
        ...
        const { testStagedModule } = await import("./test-pipeline.js");
    ...
    registry.register("cli_promote_module", async (args) => {
      try {
        ...
        const { promoteModule } = await import("./test-pipeline.js");
    ...
    registry.register("cli_list_modules", async () => {
      try {
        const { listCustomModules } = await import("./test-pipeline.js");
        const { getLoadedModules } = await import("./custom-loader.js");
        ...
        if (modules.length === 0) {
          return toolResult(
            "cli_list_modules",
            true,
            "No custom modules found. Use cli_run to generate one in custom/staging/.",
          );
        }
        ...
        const summary = modules
          .map((m) => {
            ...
            const tools = info
              ? ` (${info.toolCount} tools: ${info.toolNames.join(", ")})`
              : "";
        ...
        return toolResult(
          "cli_list_modules",
          true,
          `${modules.length} custom module(s):\n${summary}`,
        );
      } catch (err: any) {
    ...
    registry.register("cli_triage", async (args) => {
      try {
        ...
        if (!request)
          return toolResult("cli_triage", false, 'Missing "request" parameter');

        const { getAllToolsWithModules } =
        ...
        const toolSummary = allTools
          .map(
            (t: any) => `${t.tool.name} (${t.module}): ${t.tool.description}`,
          )
          .join("\n");
        const customSummary =
          customModules.length > 0
            ? customModules
                .map((m) => `${m.name}: ${m.toolNames.join(", ")}`)
                .join("\n")
            : "(none)";
        ...
        const agent = _ctx.getAgent();
        if (!agent)
          return toolResult("cli_triage", false, "Agent not initialized");
...
6. Be specific in suggestion about WHICH tools to use or HOW to implement it`;
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            const tools =
              parsed.existing_tools?.length > 0
    ...
    registry.register("cli_delete_module", async (args) => {
      try {
        ...
        const { deleteModule } = await import("./test-pipeline.js");
        const result = await deleteModule(
          moduleName,
          target as "staging" | "live" | "both",
          registry as any,
        );
        logCapability({
          action: "delete",

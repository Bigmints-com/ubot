 ...
 */
export const CLI_PROVIDER_DEFAULTS: Record<
  string,
  { command: string; package: string; authArgs: string[] }
> = {
  gemini: {
    command: "gemini",
...
export function resolveCliPackage(
  provider: string,
  configOverride?: string,
): string {
...
export function resolveCliAuthArgs(
  provider: string,
  configOverride?: string[],
): { cmd: string; args: string[] } {
  const cmd = resolveCliCommand(provider);
  const args = configOverride ||
    CLI_PROVIDER_DEFAULTS[provider]?.authArgs || ["--help"];
  return { cmd, args };
}

...
function getModelsDir(): string {
  ...
  return join(ubotHome, 'data', 'models', 'transcription');
}
...
export async function transcribeAudio(
  ...
  const baseUrl = options.providerBaseUrl || (globalThis as any).__ubotConfig?.llmBaseUrl;
  const apiKey = options.providerApiKey || (globalThis as any).__ubotConfig?.llmApiKey;
  ...
  if (isTranscriptionAvailable()) {
    try {
      ...
      const ext = extname(filePath).toLowerCase();
      const tmpDir = join(process.env.UBOT_HOME || process.cwd(), 'workspace', 'tmp');

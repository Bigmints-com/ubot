...
const browserToolModule: ToolModule = {
  ...
  register(registry: ToolRegistry, _ctx: ToolContext) {
    ...
    registry.register('read_emails', async (args) => {
      try {
        const { getGoogleAuthClient } = await import('../google/auth.js');
        const auth = await getGoogleAuthClient();
        if (auth) {
          const { gmailList } = await import('../google/gmail.js');
    ...
    registry.register('read_calendar', async (args) => {
      try {
        ...
        const auth = await getGoogleAuthClient();
        if (auth) {
          const { calendarListEvents } = await import('../google/calendar.js');

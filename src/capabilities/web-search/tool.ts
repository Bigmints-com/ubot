...
import { serperSearch } from './adapters/serper.js';
import { extractContent } from './content-extractor.js';
import type { ToolExecutionResult } from '../../engine/types.js';
...
export async function executeWebSearch(args: WebSearchToolArgs): Promise<ToolExecutionResult> {
  ...
  try {
    ...
    const response = await serperSearch(query, { count: maxResults });
    ...
    if (deepRead) {
      ...
      for (let i = 0; i < deepReadCount; i++) {
        try {
          console.log(`[WebSearch] Deep-reading: ${results[i].link}`);
          const content = await extractContent(results[i].link);
